...
export class EmployeeController {
  ...
  //region: ASSIGN SUPERVISOR
    ...
    async getAssignedSupervisorsByServiceNumberAny(
      ...
      console.log('Fetching any supervisor for service number:', serviceNumber, 'personType:', personType);
      ...
      if (personType === 'guard') {
        // Try only Guard
        const guard = await this.guardService.findByServiceNumber(serviceNumber, organizationId);
        if (!guard) {
        ...
        employeeId = await this.guardService.resolveEmployeeIdForGuard(guard.id, organizationId, false);
        if (employeeId) {
          const guardUser = await this.prisma.guard.findUnique({
            where: { id: guard.id },
            select: { user: true }
          });
          const hasUserRole = await this.employeeService.hasSupervisorRole(guardUser?.user?.id || null);
          if (hasUserRole) {
            const assignments = await this.employeeService.getAssignmentsBySupervisorId(employeeId, organizationId);
            return { data: assignments };
          }
        }
        const supervisors = await this.employeeService.getAssignedSupervisorsByEmployeeId(employeeId || '', organizationId);
        return { data: supervisors };
      } else if (personType === 'employee') {
        // Try only Employee
        const employee = await this.employeeService.findByServiceNumber(serviceNumber, organizationId);
        if (!employee) {
          throw new NotFoundException(`Employee with service number ${serviceNumber} not found`);
        }
        detectedType = 'employee';
        employeeId = employee.id;

        // For employees, check if they are a supervisor and return their assignments
        const hasUserRole = await this.employeeService.hasSupervisorRole(employee.userId);
        if (hasUserRole) {
          // Get their assignments as a supervisor
          const assignments = await this.employeeService.getAssignmentsBySupervisorId(employee.id, organizationId);
          return { data: assignments };
        } else {
          // Get who supervises them
          const supervisors = await this.employeeService.getAssignedSupervisorsByEmployeeId(employee.id, organizationId);
          return { data: supervisors };
        }
      } else {
        ...
        try {
          ...
          if (guard) {
            detectedType = 'guard';
            const resolvedId = await this.guardService.resolveEmployeeIdForGuard(guard.id, organizationId, false);
            if (!resolvedId) {
              return { data: [] };
            }
            ...
            if (employee) {
              ...
              throw new NotFoundException(`No guard or employee found with service number ${serviceNumber}`);
            }
          }
        } catch (error) {
          ...
          throw new NotFoundException(`Error retrieving person with service number ${serviceNumber}: ${error.message}`);
        }
      }
